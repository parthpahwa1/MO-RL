...
def train(env, agent, args):
    ...
    save_file_name = 'multihead3_Q_without_exemplar_' + args.env_name + '_' + str(datetime.today().strftime("%Y_%m_%d"))

    init_log_file(log_file_str)
    fixed_probe = FloatTensor([0.98, 0.02])
    env.reset()
    alpha = args.alpha
    ...
    for num_eps in range(args.episode_num+1):
        ...
        probe = np.abs(np.random.randn(2))
        ...
        if q_loss < min_loss and num_eps>1000:
            min_loss = q_loss
            agent.save(save_loc, save_file_name+"minloss_"+str(min_loss)+"_eps_"+str(num_eps))

        # if act_1 >= 17 and num_eps>500:
        #     agent.save(save_loc, save_file_name+"qval_"+str(act_1)+"_eps_"+str(num_eps))
