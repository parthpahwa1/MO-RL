...
model_loc = '../saved_models/multihead3_Q_no_wLSE_exemplar_ft_2022_06_24_eps_1999.pkl'
# # model_loc = '../saved_models/multihead3_Q_ft_2022_06_23_min_loss1_tensor(0.0086).pkl'
# model_loc = '../saved_models/multihead3_Q_ft_2022_06_23_min_loss2_tensor(0.0558).pkl'
# model_loc = '../saved_models/multihead3_Q_eps4_up75_log_ft_2022_06_23minloss_tensor(0.0615)_eps_2746.pkl'

# model_loc = '../saved_models/multihead3_Q_no_wLSE_ft7_2022_06_23_eps_1499.pkl'
# model_loc = '../saved_models/multihead3_Q_ft7_2022_06_23_min_loss1_tensor(0.0132).pkl'
# model_loc = '../saved_models/multihead3_Q_ft7_2022_06_23_min_loss2_tensor(0.0721).pkl'
# model_loc = '../saved_models/multihead3_Q_eps4_up75_log_ft7_2022_06_23minloss_tensor(0.0888)_eps_2946.pkl'

# model_loc = '../saved_models/multihead3_Q_no_wLSE_ft5_2022_06_23_eps_1499.pkl'
# model_loc = '../saved_models/multihead3_Q_ft5_2022_06_23_min_loss1_tensor(0.0132).pkl'
# model_loc = '../saved_models/multihead3_Q_ft5_2022_06_23_min_loss2_tensor(0.0721).pkl'
# model_loc = '../saved_models/multihead3_Q_eps4_up75_log_ft5_2022_06_23minloss_tensor(0.0314)_eps_2778.pkl'
...
if args.pltcontrol:
    ...
    for i in range(int(TEST_N)):
        ...
        # w = np.random.dirichlet(w)
        # w = np.random.dirichlet(np.ones(2))
        # w_e = w / np.linalg.norm(w, ord=2)
        
        probe = np.random.randn(6)
        probe = np.abs(probe)/np.linalg.norm(probe, ord=1)
        w_e = probe
        w = w_e
        if i%2 == 0:
            preference = np.random.uniform(size=6) 
            alpha = 1
            preference = np.array(preference)
            preference += 1e-6
            w_e = np.random.dirichlet(alpha*preference)
            w = w_e
    ...
    print('Policy AC:', 1/(1 + 10*policy_loss/100))
    print('Prediction AC:', 1/(1 + 10*predict_loss/100))
...
if args.pltpareto:
    ...
    for i in range(2000):
        # if i % 15 == 0 :
        #     w = np.random.uniform(size=6)/3
        # else:
        #     w = np.random.uniform(size=6)
        
        # w = np.ones(shape=len(env.reward_spec))
        # if i % 15 == 0 :
        #     w = np.random.uniform(size=6)/5
        # w = np.random.dirichlet(w)
