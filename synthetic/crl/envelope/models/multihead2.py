...
class EnvelopeLinearCQN(torch.nn.Module):
    ...
    def __init__(self, state_size, action_size, reward_size):
        ...
        self.affine4 = nn.Linear((state_size + reward_size) * 64,
                                 (state_size + reward_size) * 32)
        self.Q = nn.Linear((state_size + reward_size) * 32,
                                 action_size * reward_size)
        
        self.affine5 = nn.Linear((state_size + reward_size) * 32 + action_size * reward_size + reward_size,
                                 (state_size + reward_size) * 32)
        self.affine6 = nn.Linear((state_size + reward_size) * 32,
                                 (state_size + reward_size) * 32)

        self.q_scalar = nn.Linear((state_size + reward_size) * 32,
                                 action_size)
    ...
    def forward(self, state, preference):
        ...
        x = F.relu(self.affine5(torch.cat((x, intermediate), dim=1)))
