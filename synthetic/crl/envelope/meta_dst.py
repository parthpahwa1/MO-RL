...
class MetaAgent(object):
    ...
    def learn(self):
        if len(self.trans_mem) > self.batch_size:
            ...
            exploration_reward = torch.clamp(exploration_reward, 0, 0.1)
            ...
            loss1 = F.mse_loss(Q_scalar.gather(1, actions.unsqueeze(dim=1)), Tau_Q_scalar.view(-1).unsqueeze(dim=1))
            loss2 = F.mse_loss(Q.view(-1), Tau_Q.view(-1))
